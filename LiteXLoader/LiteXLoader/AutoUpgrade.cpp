...
DownloadResult DownloadAndCheckMd5(const string& url, const string& localPath, bool isBinary, int* errorCode = nullptr, string* md5Rtn = nullptr)
{
    ...
    cli.set_connection_timeout(LXL_UPDATE_CONNECTION_TIMEOUT, 0);
    if (!cli.is_valid())
    {
        if (errorCode)
            *errorCode = -1;
...
bool CheckAutoUpdate(bool isUpdateManually, bool forceUpdate)
{
    try
    {
        ...
        int status = -1;
        string idJson;
        if (!HttpGetSync(LXL_RELAY_INDEX, &status, &idJson, LXL_UPDATE_CONNECTION_TIMEOUT) || status != 200)
        {
            if (isUpdateManually)
        ...
        //Get Json
        string infoUrl = string(LXL_UPDATE_URL_PREFIX) + "/" + id + LXL_UPDATE_URL_PATH;
        string info;
        if (!HttpGetSync(infoUrl, &status, &info, LXL_UPDATE_CONNECTION_TIMEOUT) || status != 200)
        {
            if (isUpdateManually)
        ...
        auto res = iniVersions->LoadFile(LXL_UPDATE_OTHER_FILES_RECORD);
        if (res < 0)
        {
        ...
        else
        {
            for (auto& file : data["OtherFiles"])
            {
                ...
                if (nowVersion < newVersion)
                {
                    ...
                    string remotePath = string(LXL_UPDATE_URL_PREFIX) + "/" + id + path + fileName;
            ...
            iniVersions->SaveFile(LXL_UPDATE_OTHER_FILES_RECORD);
            delete iniVersions;
        }
        ...
        {
            autoUpgradeLogger.info("Your BDS does not match the current mainline LiteXLoader. Auto-update will not be pushed.");
            if (isUpdateManually)
                return false;
            else
                _endthreadex(0);
        ...
        else
        {
            ...
            Version verCurrent = LXL_VERSION;
            ...
            ini->SetUnicode(true);
            auto res = ini->LoadFile(LXL_UPDATE_INFO_RECORD);
            if (res >= 0 && Version::parse(ini->GetValue("Info", "Version", "0.0.0")) == verRemote)
            {
                if (isUpdateManually)
                    autoUpgradeLogger.info("Auto-update download is completed, please restart the server to finish update.");
                return true;
            }
            delete ini;
            ...
            autoUpgradeLogger.info("Congratulations! The update download is complete. Restart the server now to enjoy the new version.");
        }
    }
